...
class DecisionTreeClassifier:
    ...
    def __decisionTree(self, X, Y, features, level, metric, classes):
        ...
        if len(set(Y)) == 1:  # BASE condition 1 (if all of the Y values are the same)
            print("level", level)
            output = None
        ...
        if len(features) == 0:  # BASE condition 2 (if we run out of features to split on)
            output = None
            freq = self.__count__unique(Y)
            max_count = -math.inf
            for i in classes:
            ...
            return TreeNode(None, output)  #Output is majority class value
        max_gain = -math.inf
        final_feature = None
...
def checkAutism():
    ...
    X1_aut = df1[["A9", "A5", "A6", "A8", "A1", "A3","A7", "A2", "A4","A10"]]
    X1_aut = np.array(X1_aut)
    res=sklearnModel.predict(X1_aut)
    print(res[0])

    return jsonify({"result":int(res[0]) })
